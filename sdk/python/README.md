...
tar -zcvf pulumi-resource-clickhouse-v0.0.28-darwin-arm64.tar.gz pulumi-resource-clickhouse
gsutil cp pulumi-resource-clickhouse-v0.0.28-darwin-arm64.tar.gz gs://pulumi-shofifi/clickhouse/
cd ../..
cd dist/pulumi-clickhouse_linux_amd64_v1
tar -zcvf pulumi-resource-clickhouse-v0.0.28-linux-amd64.tar.gz pulumi-resource-clickhouse
gsutil cp pulumi-resource-clickhouse-v0.0.28-linux-amd64.tar.gz gs://pulumi-shofifi/clickhouse/
cd ../..
```
...
1. Run the following command to update files to use the name of your provider (third-party: use your GitHub organization/username):
   ...
   - replace all instances of the `github.com/Triple-Whale/pulumi-clickhouse` repository with the `REPOSITORY` location
